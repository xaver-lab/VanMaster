...
- 2026-09-19 Ansicht `#/einkauf` — `camper buy next` war der letzte Befehl ohne Ansicht. Dafür `parts.buy_next()` in `buy_daten()` (Daten, auch für `--json` und `/api/daten`) und `buy_next()` (Text) getrennt, damit Befehl und Ansicht zwingend dieselbe Reihenfolge zeigen. Die Ansicht bündelt nach Händler und kann einen ganzen Korb in einem Zug von „Entschieden" auf „Bestellt" setzen (nacheinander, nicht parallel — jede Antwort trägt die neue Dateiversion). `tests/test_einkauf.py`, 7 Tests.
- 2026-09-19 Paket C angefangen: `BereichDetail` zeigte im Reiter „Teile" eine tote Liste; jetzt die echte `TeileListe` mit neuer Eigenschaft `kategorie` (feste Kategorie, keine Auswahl, Neuanlage im Bereich, Detail ohne Adresswechsel) — dasselbe Muster wie `EinzelteilListe bereich=`. `AufgabeZeile` hatte Kästchen und Status-Auswahl für denselben Zustand; jetzt Kästchen für offen↔erledigt und Statusmarke als Weg ins Detail, wo die übrigen Status schon standen. Dabei fiel eine unscoped `:global(.status-wahl)`-Regel weg, die aus der Aufgabenzeile in alle anderen Ansichten leckte. Dazu `scroll-padding-top` auf `html`, sonst landet jeder programmatische Sprung unter der klebenden Kopfzeile.
...
- 2026-09-19 `camper verlauf` hat seine Dashboard-Ansicht bekommen (Reiter „Verlauf“ in `#/bilanz`) — der letzte Befehl ohne Ansicht, damit ist PLAN.md Stufe 3 „Kostenverlauf“ abgehakt. Kein neues Format: `data/verlauf.csv` stand schon (FORMAT.md §12), es fehlte nur die Anzeige. `tools/verlauf.py` ist dafür wie `parts.buy_daten()` in `daten()` (Zahlen) und `text()` (Ausgabe) getrennt — Befehl, `--json` und Ansicht rechnen zwingend dieselbe Zeitreihe. `daten(limit=n)` kürzt nur die Punkte; die Veränderung geht immer über den ganzen Verlauf, sonst hinge sie am Ausschnitt. Kaputte Zahlen in der CSV werden zu 0, statt die Ansicht zu sprengen. Im Diagramm liegen nur die drei Geldreihen zusammen — gleiche Einheit, gleicher Maßstab; Aufgabenstand und Gewicht bekommen keine zweite Achse danebengelegt, sie stehen in der Tabelle darunter. Unterschieden werden die Reihen über die Strichart (durchgezogen, gestrichelt, gepunktet), nicht über Farbe: das Designsystem kennt genau eine Signalfarbe, und Strichart trägt auch im Ausdruck und bei Farbsehschwäche. 5 Tests in `test_verlauf.py`, einer in `test_server.py`.
